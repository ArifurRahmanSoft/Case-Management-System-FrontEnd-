...
@Component({
    selector: 'modal',
    templateUrl: 'modal_journal.html',
  })
  export class ModalJournal implements OnInit{
    ...
    constructor(public dialogRef: MatDialogRef<ModalJournal>, @Inject(MAT_DIALOG_DATA) public data: DialogData, private api: ApiService, 
    ...
    ngOnInit(){
      ...
      this.getVoucherDetailsForJounnal(this.vNo);
    ...
    public getVoucherDetailsForJounnal(voucherNo){
  
      this.api.getJournalVoucherDetails(voucherNo)
      .subscribe((res : any[])=>{
        res.forEach(obj =>{
          ...
          this.checkedBy = obj.CHECKEDBY;
          this.preparedBy = obj.PREPAREDBY;
          this.approvedBy = obj.APPROVEBYED;
    ...
    public getVoucherApprovalMessage(voucherNo, userId){
      this.api.getVoucherApprove(voucherNo, userId)
